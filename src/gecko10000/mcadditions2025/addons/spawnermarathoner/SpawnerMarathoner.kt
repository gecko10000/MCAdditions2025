package gecko10000.mcadditions2025.addons.spawnermarathoner
...
class SpawnerMarathoner : Listener, MyKoinComponent {
    ...
    private val config: Config
        get() = plugin.config.spawnerMarathoner
    ...
    fun VehicleBlockCollisionEvent.onMinecartHitSpawner() {
        ...
        minecart.world.spawn(hitBlock.location.add(0.5, 0.0, 0.5), SpawnerMinecart::class.java) { m ->
            ...
            m.requiredPlayerRange = config.spawnerMinecartRequiredPlayerRange
            ...
            m.customName(config.spawnerMinecartCustomName(spawner.spawnedEntity))
    ...
    private fun isValidPlaceLocation(location: Location): Boolean {
        return location.block.type in config.validPlacementBlocks
    }
    ...
    private fun BlockBreakEvent.onBreakSpawner() {
        ...
        val task = if (config.spawnerBreakConfirmationEnabled)
            Task.syncDelayed(
                { -> confirmations.remove(uuid) },
                config.spawnerBreakConfirmationTimeoutTicks
            ) else null
        confirmations.put(uuid, task to this.block)
        this.player.showTitle(
            Title.title(
                config.spawnerBreakConfirmationMessageTitle,
                config.spawnerBreakConfirmationMessageSubtitle
            )
        )
        this.isCancelled = true
    }
    ...
    private fun SpawnerSpawnEvent.onMinecartSpawnerSpawn() {
        ...
        val range = config.nearbySpawnerMinecartsCheck.toDouble()
